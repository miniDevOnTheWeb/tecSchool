      ...
      <table className="table">
        ...
        <tbody>
          {students && students.map(student => (
            <tr key={student.info.id} className="row">
              <td>{student.info.name}</td>
              <td className="small-td">{student.info.age}</td>
              <td className="small-td">{student.info.age}</td>
              <td className="see-button-td small-d-td">
                <button className="global-button" onClick={() => navigate(`/teacher-dashboard/students/${student.info.id}`)}><RigthArrowIcon width={20} /></button>
              </td>
            </tr>
          ))}
        </tbody>
